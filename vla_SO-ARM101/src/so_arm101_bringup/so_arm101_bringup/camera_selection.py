"""Interactive camera picker used when --camera-id is omitted.

Copied from aruco_camera_localizer so this package stays standalone.
"""
...
def select_camera(available_ids):
    ...
    for cam_id in available_ids:
        ...
        print(
            f"Showing preview for camera ID {cam_id} "
            "(press any key to skip, ESC to select)..."
        )
        while True:
            ret, frame = cap.read()
            if not ret:
                break
            cv2.putText(frame, f"PREVIEW OF CAMERA {cam_id}", (50, 50),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            cv2.putText(frame, "ESC = SELECT, any key = SKIP", (50, 100),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
